...
public class Money implements Comparable {
	...
	/**
	 * Devuelve un String "cantidad nombre de divisa", e.g. "10.5 SEK". Los numeros
	 * decimales se representan con enteros, e.g "10.5 SEK" se almacena como 1050
	 * 
	 ...
	 * @return Un nuevo objeto Money con la divisa del actual, y la cantidad
	 *         acumulada con el importe del objeto Money recibido.
	 **/
	public Money add(Money otra) {
		// TODO: posible fallo
		return new Money(this.getCantidad() + otra.getCantidad(), this.getDivisa());
	}
	 ...
	 */
	public Money negate() {
		return new Money(-this.getCantidad(), this.divisa);
	}

	/**
	 * Compara dos objetos Money
	 * 
	 * @return 0 si sus valores son iguales Un entero negativo si el objeto actual
	 *         tiene menos valor que el recibido Un entero positivo si el objeto
	 *         actual tiene más valor que el recibido
	 */
	public int compareTo(Money otra) {
		if(this.equals(otra)){
			return 0;
		} else {
			return this.valorUniversal() > otra.valorUniversal() ? 1 : -1;
		}
	}
}